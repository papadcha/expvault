...
- `parse_csv_multi(text)`: `csv.Sniffer` για auto-detect delimiter, ομαδοποίηση γραμμών ανά
  `arithmos_parstatikou` (ή `__row_N__` fallback key αν λείπει, ώστε κάθε γραμμή χωρίς
  αριθμό να γίνεται δικό της "παραστατικό" αντί να συγχωνεύεται λάθος με το προηγούμενο).
