...
def export_pdf(kiniseis: list, yliko_label: str, period_label: str) -> bytes:
    ...
    for k in kiniseis:
        ...
        rows_data.append([
            ...
            f"{eisagogi:.3f} {k['monada_metrisis']}" if eisagogi != '' else '',
            f"{exagogi:.3f} {k['monada_metrisis']}"  if exagogi != ''  else '',
            f"{ypoloipo:.3f} {k['monada_metrisis']}",
            k.get('ypografi') or '',
        ])
    ...
    t.setStyle(TableStyle([
        ...
        ('ALIGN',         (0,1), (0,-1), 'CENTER'),   # Α/Α
        ('ALIGN',         (1,1), (1,-1), 'CENTER'),   # Ημερ
        ('ALIGN',         (6,1), (8,-1), 'RIGHT'),    # Ποσότητες
...
def export_excel(kiniseis: list, yliko_label: str, period_label: str) -> bytes:
    ...
    for row_idx, k in enumerate(kiniseis, 5):
        ...
        if eisagogi: ypoloipo += eisagogi
        else:        ypoloipo -= exagogi
        ...
        row_vals = [
            ...
            k.get('promitheftis_onoma') or '',
            k.get('arithmos_adeias') or '',
            eisagogi, exagogi, round(ypoloipo, 3),
            k.get('ypografi') or ''
        ]
        for col, val in enumerate(row_vals, 1):
            ...
            # Χρώμα υπολοίπου αν αρνητικό
            if col == 9 and ypoloipo < 0:
