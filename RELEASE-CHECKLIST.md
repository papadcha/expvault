...
**Από την v1.2.0 δεν υπάρχει πια `electron-updater`** (αφαιρέθηκε εντελώς —
silent auto-download/auto-install βασισμένο σε "ό,τι λέει το GitHub Releases
API *latest*", χωρίς καμία γνώση για known-bad releases· βλ. VERSIONS.md
v1.2.0). Για εγκαταστάσεις **≥1.2.0**, το `allowed-versions.json` είναι ο
**μοναδικός** μηχανισμός — ενημέρωσέ το σωστά (βήματα 2-3 παρακάτω) και
αρκεί, καμία αυτόματη λήψη/εγκατάσταση δεν συμβαίνει ποτέ χωρίς χειροκίνητο
κλικ.

Για εγκαταστάσεις **≤1.1.12** (ακόμα με το παλιό `electron-updater`), το
βήμα 1 παραμένει απαραίτητο — χωρίς αυτό, το `electron-updater` θα συνεχίσει
να προωθεί/εγκαθιστά αυτόματα το προβληματικό release σε όσους δεν έχουν
ενημερωθεί ακόμα, ό,τι κι αν λέει το `allowed-versions.json`.

Βήματα:
1. **(Μόνο αν επηρεάζει και ≤1.1.12 εγκαταστάσεις)** Στο GitHub → Releases →
   το προβληματικό release → **Edit** → σημείωσέ το ως **"Set as a
   pre-release"** (ή κατέβασέ το εντελώς αν είναι πολύ πρόσφατο) ώστε να
   πάψει να είναι το "latest" για το παλιό electron-updater feed. Καλή
   πρακτική να το κάνεις πάντα ούτως ή άλλως — ένα release που δείχνει σαν
   "Latest" στη δημόσια σελίδα GitHub μπορεί να κατέβει χειροκίνητα από
   εκεί ανεξάρτητα από το `allowed-versions.json`.
...
4. Αν έχει ήδη κυκλοφορήσει σε κάποιους χρήστες: θα δουν το rollback
   ...
   ελέγχεται ~3s μετά το άνοιγμα του παραθύρου).
