...
2. `allowed-versions.json` — πρόσθεσε τη νέα έκδοση στο `versions[]` με το
   πραγματικό `downloadUrl` (μορφή:
   `https://github.com/papadcha/expvault/releases/download/vX.Y.Z/ExpVault.Setup.X.Y.Z.exe`),
   και ενημέρωσε `latestRecommendedVersion` σε αυτήν — ΕΚΤΟΣ αν η νέα
   έκδοση θεωρείται δοκιμαστική/όχι έτοιμη για όλους.
